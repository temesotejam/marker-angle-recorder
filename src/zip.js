...
async function makeZip(files){
  ...
  for(const f of files){
    const name=te.encode(f.name),data=await bytesOf(f.data),crc=crc32(data);
    if(data.length>0xffffffff)throw new Error(`ZIP32 file too large: ${f.name}`);
    const local=new Uint8Array(30+name.length+data.length);
    u32(local,0,0x04034b50);u16(local,4,20);u16(local,6,0x0800);u16(local,8,0);u32(local,14,crc);u32(local,18,data.length);u32(local,22,data.length);u16(local,26,name.length);local.set(name,30);local.set(data,30+name.length);locals.push(local);
    const c=new Uint8Array(46+name.length);u32(c,0,0x02014b50);u16(c,4,20);u16(c,6,20);u16(c,8,0x0800);u32(c,16,crc);u32(c,20,data.length);u32(c,24,data.length);u16(c,28,name.length);u32(c,42,offset);c.set(name,46);centrals.push(c);offset+=local.length;
  }
